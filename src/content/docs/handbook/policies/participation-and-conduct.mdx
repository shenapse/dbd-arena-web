---
title: Participation and Conduct
description: Eligibility, participation, fair play, violations, remedies, and consequences.
status: active
version: "1.0"
effectiveDate: "2026-06-15"
changeHistory:
  - version: "1.0"
    date: "2026-06-15"
    note: Initial policy.
---

import LifecycleMeta from '@components/LifecycleMeta.astro';
import StatusBadge from '@components/StatusBadge.astro';

<StatusBadge status={frontmatter.status} />

<LifecycleMeta
  status={frontmatter.status}
  version={frontmatter.version}
  effectiveDate={frontmatter.effectiveDate}
  changeHistory={frontmatter.changeHistory}
/>

How players are expected to behave, and what happens when those expectations are broken.

## How to behave

By taking part, you agree to play fairly and treat others with respect. In particular:

- **No cheating or exploiting.** No hacks, third-party tools, bugs, or anything that gives an unfair advantage.
- **No harassment or abuse.** Treat other players and staff with respect. Hate speech, threats, and targeted harassment are not tolerated.
- **Follow and respect Arena rules.** Play by the rules of the regulation you queued for, and report results truthfully.
- **Cooperate with players and staff.** Do what is reasonably needed to keep matches moving smoothly and to resolve problems quickly.

## Recommendation: keep evidence

This is not a requirement, but players are encouraged to keep evidence that can help resolve trouble quickly. Recording matches, saving clips, or keeping screenshots can make it much easier to understand what happened if a dispute or conduct issue comes up.

## Warning and disciplinary actions

When the rules are broken, the response is matched to how serious and how repeated the behaviour is. Typical categories, from least to most serious, are:

- **A warning** for minor or first-time issues.
- **A temporary restriction**, such as a cooldown before you can queue again.
- **A suspension** from the service for a period.
- **Removal** from the service for serious or repeated violations.

These are the kinds of consequences you can expect; the exact response in any case is decided by staff. Cheating, harassment, and deliberate result manipulation are treated as serious and may skip straight to the stronger responses.

## Staff discretion

Staff weigh the evidence, the context, and any history before deciding, and they hold final discretion over the outcome of any individual case. 
